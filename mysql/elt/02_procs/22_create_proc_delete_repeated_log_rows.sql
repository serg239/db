/*
  Version:
    2011.11.12.01
  Script:
    22_create_proc_delete_repeated_log_rows.sql
  Description:
  ...
  Output:
    * error_code: 
      ...
      * < 0 - Error 
  Install:
    mysql -h <db_name> -u <db_user_name> -p<db_user_pwd> < <path_to_file>\22_create_proc_delete_repeated_log_rows.sql
  Usage:
    CALL elt.delete_repeated_rows (@err, 'STAGE', FALSE);    -- from all STAGE tables, first download
    CALL elt.delete_repeated_rows (@err, 'DELTA', FALSE);    -- from all DELTA tables, last download
  Example:
...
BEGIN
  ...
  DECLARE v_log_table_name            VARCHAR(64);     -- LOG table name
  DECLARE v_log_table_pk_column_name  VARCHAR(64);     -- Name of the PK in LOG Table
  DECLARE v_dtm_column_name           VARCHAR(64);     -- log_modified_dtm OR log_created_dtm
  DECLARE v_dedup_block_size          INTEGER;
  DECLARE v_proc_pk_column_name       VARCHAR(64);     -- Name of the PK in the BASE Table
  ...
  -- if table name is not defined - get all LOG tables in current schema
  -- and PK Column Name of BASE and LOG Objects
  DECLARE log_tables_cur CURSOR
  FOR
  SELECT src.src_table_name                   AS log_table_name,
    ...
    FROM elt.src_tables                      src
      ...
      INNER JOIN information_schema.columns  isc
        ON src.src_table_name  = isc.table_name    
          AND isc.table_schema = v_dst_schema_name   -- schema: 'db_stage' or 'db_delta'
  ...
  -- statement for grouped columns of the LOG table
    ...
    FROM 
      (SELECT GROUP_CONCAT(''''''''', IFNULL(', column_name, ',', '''NULL''), '''''',''') AS tbl_cols
        ...
        WHERE table_schema = v_dst_schema_name
          AND table_name   = v_log_table_name
  ...
  BEGIN
    ...
    --
    IF UPPER(schema_type_in) = STAGE_SCHEMA_TYPE THEN
      SET v_dst_schema_name = STAGE_SCHEMA_NAME;
    ELSEIF UPPER(schema_type_in) = DELTA_SCHEMA_TYPE THEN
    ...
    IF (v_download_num > 0) THEN
      ...
      SET @sql_stmt = CONCAT('CREATE TABLE IF NOT EXISTS ', PROC_SCHEMA_NAME, '.', v_tmp_filter_pk_tbl_name,' (pk_id  BIGINT) ENGINE = InnoDB');
...
--    SET @@GLOBAL.SLOW_QUERY_LOG = 'OFF';     -- SET @@GLOBAL.SLOW_QUERY_LOG = 0;
      ...
      LOOP
        ...
        FETCH log_tables_cur                 -- WHERE v_dst_schema_name
        INTO v_log_table_name, 
...
/*
  +----------------------------------------+-------------------------------------------+------------------+------------------+---------------------------------------+
  | log_table_name                         | log_table_pk_column_name                  | dtm_column_name  | dedup_block_size | proc_pk_column_name                   |
  +----------------------------------------+-------------------------------------------+------------------+------------------+---------------------------------------+
  | account_log                            | account_log_id                            | log_modified_dtm |            10000 | account_id                            |
  | attribute_label_group_log              | attribute_label_group_log_id              | log_created_dtm  |            10000 | attribute_label_group_id              |
  | attribute_log                          | attribute_log_id                          | log_created_dtm  |            10000 | attribute_id                          |
  | attribute_value_log                    | attribute_value_log_id                    | log_created_dtm  |            10000 | attribute_value_id                    |
  | brand_log                              | brand_log_id                              | log_created_dtm  |            10000 | brand_id                              |
  | content_item_asset_log                 | content_item_asset_log_id                 | log_created_dtm  |           100000 | content_item_asset_id                 |
  | content_item_build_instruction_log     | content_item_build_instruction_log_id     | log_created_dtm  |           100000 | content_item_build_instruction_id     |
  | content_item_class_attribute_log       | content_item_class_attribute_log_id       | log_created_dtm  |            10000 | content_item_class_attribute_id       |
  | content_item_class_hierarchy_log       | content_item_class_hierarchy_log_id       | log_created_dtm  |            10000 | content_item_class_hierarchy_id       |
  | content_item_class_log                 | content_item_class_log_id                 | log_created_dtm  |            10000 | content_item_class_id                 |
  | content_item_external_item_mapping_log | content_item_external_item_mapping_log_id | log_created_dtm  |           100000 | content_item_external_item_mapping_id |
  | content_item_log                       | content_item_log_id                       | log_created_dtm  |           100000 | content_item_id                       |
  | hierarchy_log                          | hierarchy_log_id                          | log_created_dtm  |            10000 | hierarchy_id                          |
  | user_log                               | user_log_id                               | log_modified_dtm |            10000 | user_id                               |
  | value_log                              | value_log_id                              | log_created_dtm  |            10000 | value_id                              |
  +----------------------------------------+-------------------------------------------+------------------+------------------+---------------------------------------+
*/
        ...
        IF NOT done THEN
          ...
          -- ==============================
          OPEN obj_columns_stmt_cur;
          FETCH obj_columns_stmt_cur INTO v_column_str;         -- WHERE v_dst_schema_name AND v_log_table_name
          IF debug_mode_in THEN
          ...
          LOOP
            ...
            -- Truncate Temp Table
            SET @sql_stmt = CONCAT('TRUNCATE TABLE ', PROC_SCHEMA_NAME, '.', v_tmp_filter_pk_tbl_name);
            IF debug_mode_in THEN
              SELECT CONCAT(CAST(@sql_stmt AS CHAR), ";", @LF) AS debug_sql;
            ELSE
              PREPARE query FROM @sql_stmt;
            ...
            SET @sql_stmt = CONCAT('INSERT INTO ', PROC_SCHEMA_NAME, '.', v_tmp_filter_pk_tbl_name, ' (pk_id) ', 
                                   ' SELECT DISTINCT ', v_proc_pk_column_name, ' AS pk_id',
                                   ' FROM ', v_dst_schema_name, '.', v_log_table_name, 
                                   ' ORDER BY 1', 
            ...
            IF (v_pk_num IS NOT NULL) AND (v_pk_num > 0) THEN
              ...
              LOOP
                ...
                --
                SET @sql_stmt = CONCAT('SELECT pk_id INTO @v_pk_id FROM ', PROC_SCHEMA_NAME, '.', v_tmp_filter_pk_tbl_name, ' LIMIT ', v_proc_rows_offset, ', 1'); 
                IF debug_mode_in THEN
                  SELECT CONCAT(CAST(@sql_stmt AS CHAR), ";", @LF) AS debug_sql;
                ELSE
                  PREPARE query FROM @sql_stmt;
                ...
                IF (@v_pk_id IS NOT NULL) AND (v_proc_rows_offset < v_pk_num) THEN
                  ...
                  ' FROM ', v_dst_schema_name, '.', v_log_table_name, ' log',       @LF,
                  ...
                  -- 
                  SET @sql_stmt = CONCAT(
                   'DELETE FROM ', v_dst_schema_name, '.', v_log_table_name,       @LF,
            ...
            SET @sql_stmt = CONCAT('INSERT INTO elt.repeated_log_records (download_num, schema_name, table_name, entries_num, total_rows, deleted_rows, proc_duration, created_by, created_at)', @LF,
      ...
      -- ==============================
      CLOSE log_tables_cur;

      -- Drop Table of concatenated fields' values
      DROP TABLE IF EXISTS log_table_changes;

      -- Drop Temporary Table
      SET @sql_stmt = CONCAT('DROP TABLE IF EXISTS ', PROC_SCHEMA_NAME, '.', v_tmp_filter_pk_tbl_name);
      IF debug_mode_in THEN
        SELECT CONCAT(CAST(@sql_stmt AS CHAR), ";", @LF) AS debug_sql;
      ELSE                
        PREPARE query FROM @sql_stmt;
